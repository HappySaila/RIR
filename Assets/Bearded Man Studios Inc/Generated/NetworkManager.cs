...
namespace BeardedManStudios.Forge.Networking.Unity
{
	public partial class NetworkManager : MonoBehaviour
	{
		...
		public GameObject[] RobotManagerNetworkObject = null;
		public GameObject[] RobotNetworkObject = null;
		public GameObject[] TestNetworkObject = null;
		public GameObject[] masterServerNetworkObject = null;
		...
		private void CaptureObjects(NetworkObject obj)
		{
			...
			else if (obj is RobotManagerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (RobotManagerNetworkObject.Length > 0 && RobotManagerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(RobotManagerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
			...
			else if (obj is masterServerNetworkObject)
			{
				MainThreadManager.Run(() =>
				{
					NetworkBehavior newObj = null;
					if (!NetworkBehavior.skipAttachIds.TryGetValue(obj.NetworkId, out newObj))
					{
						if (masterServerNetworkObject.Length > 0 && masterServerNetworkObject[obj.CreateCode] != null)
						{
							var go = Instantiate(masterServerNetworkObject[obj.CreateCode]);
							newObj = go.GetComponent<NetworkBehavior>();
						}
					}
		...
		[Obsolete("Use InstantiateRobotManager instead, its shorter and easier to type out ;)")]
		public RobotManagerBehavior InstantiateRobotManagerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RobotManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<RobotManagerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<RobotManagerBehavior>().networkObject = (RobotManagerNetworkObject)obj;
		...
		[Obsolete("Use InstantiatemasterServer instead, its shorter and easier to type out ;)")]
		public masterServerBehavior InstantiatemasterServerNetworkObject(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(masterServerNetworkObject[index]);
			var netBehavior = go.GetComponent<masterServerBehavior>();
			var obj = netBehavior.CreateNetworkObject(Networker, index);
			go.GetComponent<masterServerBehavior>().networkObject = (masterServerNetworkObject)obj;
		...
		public RobotManagerBehavior InstantiateRobotManager(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(RobotManagerNetworkObject[index]);
			var netBehavior = go.GetComponent<RobotManagerBehavior>();
		...
		public masterServerBehavior InstantiatemasterServer(int index = 0, Vector3? position = null, Quaternion? rotation = null, bool sendTransform = true)
		{
			var go = Instantiate(masterServerNetworkObject[index]);
			var netBehavior = go.GetComponent<masterServerBehavior>();
